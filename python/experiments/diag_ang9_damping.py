...
"""
诊断 ang9 震荡 + 阻尼方案验证 (复用 verify_condition2 的正确完整流程)
  现象: 无终止时 ang9=∠(v6,v) 逐渐减小但震荡回去
  假设: 震荡源 = 每轮旋转参考(newf/newv)导致 NM 目标移动 → d 来回摆
  方案: 阻尼 — 旋转角度 ×ω (部分旋转) / t 更新阻尼 / best-so-far 跟踪
"""
import math
import numpy as np
...
def run_traj(name, pts, omega=1.0, best_track=False, max_outer=25):
    """完整流程 + ang9 轨迹; omega=旋转阻尼系数; best_track=best-so-far"""
    ...
    for i in range(max_outer):
        ...
        if ang9 < best9:
            ...
            best_t = (t1n, t2n)
        if i % 3 == 0:
            print(f"    iter{i:2d}: ang9={ang9:6.1f}° ang5={ang5:6.1f} ang6={ang6:6.1f} "
                  f"best9={best9:6.1f}°", flush=True)
        # 2) 旋转参考 (ω 阻尼)
        axis = np.cross(v6, v2)
        axis1 = np.cross(v6, v4)
        newf = rotate(v2, axis, -ang5 * omega)
        newv = rotate(v4, axis1, -ang6 * omega)
        # 3) NM 精化
    ...
    if best_track:
        ...
        f1z, f2z = fit_fast(P, ptn, F1n, F2n, a)
        v6f = norm(f1z - f2z)
        print(f"    best-track 恢复: ang9={angle(v6f, v):.1f}°")
    return traj


def main():
    print("=" * 96)
    print("ang9 轨迹: ω=1.0(原) vs ω=0.5(半阻尼) vs best-track")
    print("=" * 96)
    for name, pts in [("ball2", dd.make_ball(2)), ("ellip0", dd.make_ellip(0))]:
        print(f"  [{name}]")
        print("    ω=1.0 (原, 无终止):")
        t1 = run_traj(name, pts, omega=1.0)
        print("    ω=0.5 (半阻尼):")
        t2 = run_traj(name, pts, omega=0.5)
        print(flush=True)
    print("DONE")

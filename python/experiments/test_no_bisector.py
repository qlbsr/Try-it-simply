...
"""
用户发现: 条件只需 |Δ| = |∠(d,v) - ∠(d,F1-F2)| ≤ 10, 且"完全与角平分线无关"。
验证: 逐步剥离组件, 量化各组件是否必要 (真实数据 pyjson.json)
  变体 B: 去掉 vj 预精化 (vj = v̂+d̂ 角平分线构造)
  变体 C: 纯 NM — 无 vj / 无 LM(RefineModuliByAxis) / 无旋转, 参考固定 v 与 (F1-F2)
         目标 |∠(d,v)-∠(d,F1-F2)|, stopF=10 → 若 1 轮达成即证明角平分线完全无关
对照: 变体 A = 用户当前全流程 (已有结果: 4 迭代, NM 1140 求值)
"""
import json
import math
import time
...
def nm_on_delta(t1_init, t2_init, pts, r30, r45, a, pcav, f1f2v,
    ...
    """NM 最小化 |Δ| = |∠(d,pcav) - ∠(d,f1f2v)|, 返回 (t1,t2,|Δ|,∠v,∠1,evals,F1z,F2z)"""
...
def load_setup():
    ...
    return points, r30, r45, a, v, F1, F2, f1, f2, F01, F02, t1t, t2t, t_


def variant_b(points, r30, r45, a, v, F1, F2, f1, f2, F01, F02, t_):
    """去掉 vj 预精化: 直接从 t_=(0,1) 起步, 其余 (LM+旋转+NM) 与用户流程相同"""
    print("=" * 70)
    print("变体 B  去掉 vj 角平分线预精化 (保留 LM + 旋转 + NM)")
    print("=" * 70)
    t1 = t2 = t_
    ...
    for i in range(300):
        ...
        print(f"iter {i:3d}: ∠(d,v)={angle_deg:7.2f}° ∠(d,F1-F2)={angle_deg1:7.2f}° "
              f"|Δ|={delta:6.2f}°  [{(time.perf_counter()-t0):5.1f}s]")
        if delta <= 10:
            print(f"== 达成 (|Δ|={delta:.2f} ≤ 10), {i} 次迭代 ==")
            break
        ...
        t1, t2 = t1_, t2_
        f1, f2 = F1z, F2z
        print(f"        NM: |Δ|={delta_deg:.2f}° (∠={ad_:.1f}°,∠1={ad1_:.1f}°, {evals}次)")
    print(f"变体 B 总耗时 {time.perf_counter()-t0:.1f}s")
    print()


def variant_c(points, r30, r45, a, v, F1, F2, f1, f2, t1t, t2t):
    """纯 NM: 无 vj / 无 LM / 无旋转. 参考固定 v 与 (F1-F2), 目标 |Δ|, stopF=10.
       若 1 轮达成 → 角平分线与所有中间构造完全无关"""
    print("=" * 70)
    print("变体 C  纯 NM: 无 vj / 无 LM / 无旋转 (参考固定 v 与 F1-F2)")
    print("=" * 70)
    t0 = time.perf_counter()
    t1, t2, delta_deg, ad_, ad1_, evals, F1z, F2z = nm_on_delta(
        t1t, t2t, points, r30, r45, a, v, F1 - F2, max_evals=600, stop_f=10.0)
    ok = "✅ 达成" if delta_deg <= 10 else "✗ 未达成"
    print(f"NM 单轮: |Δ|={delta_deg:.2f}° (∠(d,v)={ad_:.1f}°, ∠(d,F1-F2)={ad1_:.1f}°, "
          f"{evals}次求值)  t1={t1:.4f} t2={t2:.4f}  {ok}")
    print(f"变体 C 总耗时 {time.perf_counter()-t0:.1f}s")
    print()


def main():
    points, r30, r45, a, v, F1, F2, f1, f2, F01, F02, t1t, t2t, t_ = load_setup()
    print(f"数据: {len(points)} 点 | ∠(v, F1-F2)={angle(v, F1-F2):.1f}°")
    variant_b(points, r30, r45, a, v, F1, F2, f1, f2, F01, F02, t_)
    variant_c(points, r30, r45, a, v, F1, F2, f1, f2, t1t, t2t)
    print("DONE")

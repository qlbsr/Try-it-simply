...
"""
s5 趋势: 跟随哪个固定焦点场? (s1=v1理论F12, s2=v2拟合f12, s3=v3(0,1)F012, s4=v4(0,1)拟合f012)
  s5 = P(v5·c), v5 = 当前方向 (每轮变)
  全部同为 BatchProbability 焦点场族 → 比较 corr(s5, s_i) 随迭代
  以及方向角: ∠(v5, v_i)
"""
import math
import numpy as np
...
def run_track(name, pts, max_outer=15):
    ...
    v1 = norm(F1 - F2)     # 理论 taus 提取焦点方向
    v2 = norm(f1 - f2)     # 理论 taus 拟合焦点方向
    v3 = norm(F01 - F02)   # (0,1) 提取焦点方向
    ...
    s4 = bp(P, v4 * c, -v4 * c, a)
    s0 = bp(P, v * c, -v * c, a)
    refs = {"s1(thF12)": s1, "s2(fitf12)": s2, "s3(01F012)": s3,
            "s4(01fitf012)": s4, "s0(pca)": s0}
    dirs = {"v1": v1, "v2": v2, "v3": v3, "v4": v4, "v(pca)": v}

    vj = v + norm(F1 - F2)
    t1, t2 = t_, t_
    f1z, f2z = f1, f2
    print(f"  [{name}]")
    hdr = "  iter  cond | " + " ".join(f"corr(5,{k.split('(')[0]:>6s})" for k in refs)
    hdr += " | " + " ".join(f"∠(5,{k})" for k in dirs)
    print(hdr)
    for i in range(max_outer):
        ...
        s5 = bp(P, d * c, -d * c, a)
        row = f"  {i:4d} {cond:5.2f} | "
        for nm, sfix in refs.items():
            row += f"{np.corrcoef(s5, sfix)[0,1]:16.3f}"
        row += " | "
        for nm, dv in dirs.items():
            row += f"{angle(d, dv):8.1f}"
...
def main():
    print("=" * 150)
    print("s5 跟随哪个固定焦点场? corr(s5, s_i) 与 ∠(v5, v_i) 逐轮")
    print("=" * 150)

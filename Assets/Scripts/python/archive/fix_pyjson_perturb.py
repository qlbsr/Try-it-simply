...
"""
pyjson 聚焦: 60° 扰动改为"停滞检测触发"而非"angleDegnew<60"
  修复: 自然累加(angNew 单调变化)时不动; 只有 angNew 停滞才随机60°救援
  对比: ① 原 <60 触发 ② 停滞触发 ③ 完全不扰动(纯自然)
  目标: a9 能否恢复收敛 (pyjson 完美解 ∠2.3°)
"""
import json
import math
import numpy as np
...
def run_pyjson(mode, seed=0, n_rounds=80, stall_thresh=5):
    """mode: 'none' 纯自然 | 'lt60' 原<60触发 | 'stall' 停滞触发"""
    ...
    for i in range(n_rounds):
        ...
        if mode == "lt60" and angNew < 60:
            do_pert = True
        elif mode == "stall" and stall >= stall_thresh:
            do_pert = True
            stall = 0
        if do_pert:
            n_pert += 1
    ...
    return best_a9v, best_a9t, n_pert, traj


def main():
    print("=" * 100)
    print("pyjson 聚焦: 扰动触发策略对比 (80轮, 3 seeds best)")
    print("=" * 100)
    for mode, desc in [("none", "纯自然(不扰动)"), ("lt60", "原 <60 触发"),
                       ("stall", "停滞≥5轮才扰动")]:
        bv, bt = 999.0, 999.0
        for sd in range(3):
            try:
                av, at, np_, tr = run_pyjson(mode, seed=sd)
                bv = min(bv, av)
                bt = min(bt, at)
            except Exception as e:
                print(f"  {mode} seed{sd}: 失败 {e}", flush=True)
        print(f"  {desc:16s}: a9(v3)best={bv:6.1f}°  a9(真)best={bt:6.1f}°", flush=True)
    print()
    # 详细轨迹: stall 模式 seed0
    av, at, np_, tr = run_pyjson("stall", seed=0)
    print("stall 模式 seed0 轨迹 (每5轮):")
    print(f"  {'iter':>4s} {'angNew':>7s} {'a9v3':>6s} {'a9真':>6s}")
    for i, an, a9v, a9t in tr:
        if i % 5 == 0 or i == len(tr) - 1:
            print(f"  {i:4d} {an:7.1f} {a9v:6.1f} {a9t:6.1f}", flush=True)
    print(f"  扰动次数={np_}")
    print("DONE")

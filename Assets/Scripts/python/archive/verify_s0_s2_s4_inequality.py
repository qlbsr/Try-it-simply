...
"""
验证不等式: 达成时 |s2[0]-s0[0]| 与 |s4[0]-s0[0]| 均 > |s0[0]-s5[0]|
  s0 = P(v·c)[0]    PCA方向      s5 = P(d·c)[0]   当前方向
  s2 = P(v2·c)[0]   拟合焦点方向  s4 = P(v4·c)[0]  (0,1)拟合焦点方向
  意义: 当前方向 d 的概率比其他固定方向更接近 PCA 方向概率
  ellip0: 检查达成点;  ellip1: 未达成则逐轮列出详细数据
"""
import json
import math
import time
...
def run_detail(name, pts, max_outer=12):
    ...
    v3 = norm(F01 - F02)
    v4 = norm(f01 - f02)
    s0 = batch_probability(P, v * c, -v * c, a)[0]
    s2 = batch_probability(P, v2 * c, -v2 * c, a)[0]
    s4 = batch_probability(P, v4 * c, -v4 * c, a)[0]
    ...
    print(f"  [{name}] s0={s0*100:.3f} s2={s2*100:.3f} s4={s4*100:.3f} "
          f"|s2-s0|={abs(s2-s0)*100:.3f} |s4-s0|={abs(s4-s0)*100:.3f}")
    print(f"  {'iter':>4s} {'cond':>6s} {'✅':>2s} | "
          f"{'s5':>7s} {'|s0-s5|':>8s} {'|s2-s0|':>8s} {'|s4-s0|':>8s} "
          f"{'s5<s2?':>6s} {'s5<s4?':>6s}")
    for i in range(max_outer):
        ...
        d4_0 = abs(s4 - s0) * 100
        ok = cond <= 8
        # 用户理论: |s2-s0| > |s0-s5| 且 |s4-s0| > |s0-s5|
        theory = (d2_0 > d0_5) and (d4_0 > d0_5)
        print(f"  {i:4d} {cond:6.2f} {str(ok):>2s} | "
              f"{s5*100:7.3f} {d0_5:8.3f} {d2_0:8.3f} {d4_0:8.3f} "
              f"{str(s5<s2):>6s} {str(s5<s4):>6s}  theory={'✅' if theory else '✗'}",
              flush=True)
        if ok:
            print(f"    ==> 达成 iter{i}: theory={'✅成立' if theory else '✗不成立'}")
            return
        if i >= max_outer - 1:
            print(f"    ==> 未达成 (max_outer={max_outer}), theory 无达成点可验")
            return
...
def main():
    ...
    print("=" * 104)
    print("不等式验证: |s2[0]-s0[0]|>|s0[0]-s5[0]| 且 |s4[0]-s0[0]|>|s0[0]-s5[0]|")
    print("=" * 104)
